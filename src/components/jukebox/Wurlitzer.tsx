import { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";

type PlayState = "idle" | "sliding" | "rising" | "lifting" | "playing";

type Props = {
  triggerPlay?: boolean;
  triggerReset?: boolean;
  onPlayComplete?: () => void;
  onReset?: () => void;
  onNeedleDown?: () => void;
  showControls?: boolean;
  variant?: "large" | "small";
};

// Layout configurations for different variants
const LAYOUT = {
  large: {
    platterX: 210,
    stackLeft: "left-6",
    recordSize: "h-36 w-36",
    platterSize: "h-36 w-36",
    tonearmWidth: "w-20",
    tonearmRight: 5,
    spindleHeight: "h-36",
    liftColumnHeight: "h-48",
    liftColumnWidth: "w-6",
    centerDotSize: "h-4 w-4",
    tonearmHeight: "h-1.5",
    tonearmHeadSize: "h-3 w-4",
    tonearmPivotSize: "h-6 w-6",
    stackSpacing: 8,
    liftHeight: 20,
    tonearmBaseOffset: 110,
  },
  small: {
    platterX: 70,
    stackLeft: "left-2",
    recordSize: "h-20 w-20",
    platterSize: "h-20 w-20",
    tonearmWidth: "w-12",
    tonearmRight: 2,
    spindleHeight: "h-20",
    liftColumnHeight: "h-28",
    liftColumnWidth: "w-4",
    centerDotSize: "h-2.5 w-2.5",
    tonearmHeight: "h-1",
    tonearmHeadSize: "h-2 w-2.5",
    tonearmPivotSize: "h-4 w-4",
    stackSpacing: 4,
    liftHeight: 12,
    tonearmBaseOffset: 62,
  },
} as const;

export function Wurlitzer({ triggerPlay, triggerReset, onPlayComplete, onReset, onNeedleDown, showControls = true, variant = "large" }: Props) {
  const [playState, setPlayState] = useState<PlayState>("idle");
  const [currentRecord, setCurrentRecord] = useState(0);
  const layout = LAYOUT[variant];

  const recordStackY = (14 - currentRecord) * layout.stackSpacing;

  const handlePlay = useCallback(() => {
    if (playState !== "idle") return;
    setPlayState("sliding");
    setTimeout(() => setPlayState("rising"), 800);
    setTimeout(() => setPlayState("lifting"), 1400);
    setTimeout(() => setPlayState("playing"), 2000);
  }, [playState]);

  // External trigger to play
  useEffect(() => {
    if (triggerPlay && playState === "idle") {
      handlePlay();
    }
  }, [triggerPlay, playState, handlePlay]);

  // External trigger to reset
  useEffect(() => {
    if (triggerReset && playState !== "idle") {
      setPlayState("idle");
      setCurrentRecord((r) => (r + 1) % 15);
    }
  }, [triggerReset, playState]);

  // Notify when needle reaches the record
  useEffect(() => {
    if (playState === "playing") {
      onNeedleDown?.();
    }
  }, [playState, onNeedleDown]);

  // Notify when playing completes (after ~3 seconds of playing)
  useEffect(() => {
    if (playState === "playing") {
      const timer = setTimeout(() => {
        onPlayComplete?.();
      }, 3000);
      return () => clearTimeout(timer);
    }
  }, [playState, onPlayComplete]);

  return (
    <div className="relative w-full h-full overflow-hidden">
      {/* Player Stage */}
      <div
        className="relative mx-auto overflow-hidden h-full w-full"
        style={{
          perspective: "800px",
        }}
      >
        {/* 3D Stage */}
        <div
          className="relative h-full w-full"
          style={{
            transformStyle: "preserve-3d",
            transform: "rotateX(15deg)",
            transformOrigin: "center 100%",
          }}
        >
          {/* Record Stack (left) */}
          <div className={`absolute ${layout.stackLeft}`} style={{ bottom: -40 }}>
            {[...Array(15)].map((_, i) => {
              const isTop = i === currentRecord;
              const hide = isTop && playState !== "idle";
              return (
                <motion.div
                  key={i}
                  className="absolute"
                  style={{ bottom: (14 - i) * layout.stackSpacing, zIndex: 15 - i }}
                  animate={{ opacity: hide ? 0 : 1 }}
                >
                  <Record isActive={isTop} size={layout.recordSize} />
                </motion.div>
              );
            })}
            {/* Spindle */}
            <div className={`absolute bottom-0 left-1/2 ${layout.spindleHeight} w-2 -translate-x-1/2 rounded-full bg-gradient-to-b from-amber-400 to-amber-600`} />
          </div>

          {/* Sliding/Lifting Record */}
          <AnimatePresence>
            {playState !== "idle" && (
              <motion.div
                className={`absolute ${layout.stackLeft}`}
                style={{ zIndex: 15 - currentRecord, transform: "translateZ(0)" }}
                initial={{ x: 0, bottom: -40 + recordStackY }}
                animate={{
                  x: layout.platterX,
                  bottom: playState === "lifting" || playState === "playing"
                    ? -40 + recordStackY + layout.liftHeight
                    : -40 + recordStackY,
                }}
                exit={{ opacity: 0 }}
                transition={{ type: "spring", stiffness: 50, damping: 12 }}
              >
                <div className={playState === "playing" ? "animate-spin-record" : ""}>
                  <Record isActive spinning={playState === "playing"} size={layout.recordSize} />
                </div>
              </motion.div>
            )}
          </AnimatePresence>

          {/* Platter with lift column */}
          <motion.div
            className={`absolute ${layout.stackLeft}`}
            style={{ x: layout.platterX, transform: "translateZ(0)" }}
            initial={{ bottom: -40 }}
            animate={{
              bottom: playState === "rising"
                ? -40 + recordStackY
                : playState === "lifting" || playState === "playing"
                  ? -40 + recordStackY + layout.liftHeight
                  : -40,
            }}
            transition={{ type: "spring", stiffness: 50, damping: 12 }}
          >
            <div
              className={`absolute left-1/2 top-1/2 ${layout.liftColumnHeight} ${layout.liftColumnWidth} -translate-x-1/2 rounded-b-lg bg-gradient-to-b from-neutral-500 via-neutral-600 to-neutral-700`}
              style={{ boxShadow: "inset 2px 0 4px rgba(255,255,255,0.1), inset -2px 0 4px rgba(0,0,0,0.3)" }}
            />
            <div
              className={`relative ${layout.platterSize} rounded-full bg-gradient-to-b from-neutral-600 to-neutral-800`}
              style={{
                boxShadow: "0 6px 24px rgba(0,0,0,0.6)",
                transform: "rotateX(65deg)",
                transformStyle: "preserve-3d",
              }}
            >
              <div className="absolute inset-2 rounded-full bg-neutral-700" />
              <div className={`absolute left-1/2 top-1/2 ${layout.centerDotSize} -translate-x-1/2 -translate-y-1/2 rounded-full bg-amber-500`} />
            </div>
          </motion.div>

          {/* Tonearm */}
          <motion.div
            className="absolute"
            style={{
              right: layout.tonearmRight,
              bottom: -40 + recordStackY + layout.tonearmBaseOffset,
              zIndex: 50,
              transformOrigin: 'right center',
            }}
            animate={{ rotate: playState === "playing" ? 8 : 15 }}
            transition={{ type: "spring", stiffness: 80, damping: 12, delay: 0.2 }}
          >
            <div className={`${layout.tonearmHeight} ${layout.tonearmWidth} rounded-full bg-amber-500`} />
            <div className={`absolute -top-1 left-0 ${layout.tonearmHeadSize} rounded-sm bg-amber-400`} />
            <div className={`absolute -right-3 -top-3 ${layout.tonearmPivotSize} rounded-full bg-amber-600`} />
          </motion.div>
        </div>

        {/* Floor fade */}
        <div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-neutral-900 to-transparent" />
      </div>
    </div>
  );
}

function Record({ isActive = false, spinning = false, size = "h-36 w-36" }: { isActive?: boolean; spinning?: boolean; size?: string }) {
  // Determine size variant for proportional inner elements
  const isSmall = size.includes("h-20");

  return (
    <div
      className={`${size} overflow-hidden rounded-full ${isActive ? "border border-rose-800/50" : "border border-neutral-800/50"}`}
      style={{
        background: isActive
          ? "radial-gradient(circle, #1f1f1f 25%, #0a0a0a 70%)"
          : "radial-gradient(circle, #181818 25%, #080808 70%)",
        boxShadow: spinning
          ? "0 8px 32px rgba(0,0,0,0.8)"
          : "0 4px 16px rgba(0,0,0,0.6)",
        transform: "rotateX(65deg)",
        transformStyle: "preserve-3d",
      }}
    >
      {/* Groove rings */}
      {[0, 45, 90, 135].map((angle) => (
        <div
          key={angle}
          className="absolute left-1/2 top-1/2 h-0.5 w-full origin-left -translate-y-1/2"
          style={{
            transform: `rotate(${angle}deg)`,
            background: "linear-gradient(90deg, transparent 20%, rgba(255,255,255,0.08) 30%, rgba(255,255,255,0.08) 70%, transparent 80%)",
          }}
        />
      ))}
      {/* Outer groove ring */}
      <div className={`absolute left-1/2 top-1/2 ${isSmall ? "h-14 w-14" : "h-28 w-28"} -translate-x-1/2 -translate-y-1/2 rounded-full border border-neutral-700/30`} />
      {/* Inner groove ring */}
      <div className={`absolute left-1/2 top-1/2 ${isSmall ? "h-10 w-10" : "h-20 w-20"} -translate-x-1/2 -translate-y-1/2 rounded-full border border-neutral-700/20`} />
      {/* Label */}
      <div
        className={`absolute left-1/2 top-1/2 ${isSmall ? "h-6 w-6" : "h-12 w-12"} -translate-x-1/2 -translate-y-1/2 rounded-full ${
          isActive ? "bg-gradient-to-br from-rose-600 to-rose-800" : "bg-gradient-to-br from-amber-700 to-amber-900"
        }`}
      >
        {/* Label highlight */}
        <div className={`absolute ${isSmall ? "h-1 w-1" : "h-2 w-2"} rounded-full bg-white/20`} style={{ top: "25%", left: "60%" }} />
        {/* Center hole */}
        <div className={`absolute left-1/2 top-1/2 ${isSmall ? "h-2 w-2" : "h-3 w-3"} -translate-x-1/2 -translate-y-1/2 rounded-full bg-black`} />
      </div>
    </div>
  );
}
